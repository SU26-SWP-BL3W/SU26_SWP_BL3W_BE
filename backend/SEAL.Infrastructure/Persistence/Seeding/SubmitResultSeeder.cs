    ...
    public class SubmitResultSeeder : IDataSeeder
    {
        ...
        public async Task SeedAsync(DatabaseContext context)
        {
            if (!await context.SubmitResults.AnyAsync())
            {
                ...
                var track = await context.Tracks.Include(t => t.Round).FirstOrDefaultAsync(t => t.TrackName == "Technology & AI");

                if (team == null || track == null)
                {
                    _logger.LogWarning("Required Team or Track not found for SubmitResult seeding.");
                    return;
                }
