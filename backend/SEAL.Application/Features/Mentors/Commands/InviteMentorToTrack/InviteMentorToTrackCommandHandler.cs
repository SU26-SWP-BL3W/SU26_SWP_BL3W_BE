...
{
    /// <summary>
    /// Mời một mentor (đã có tài khoản) hướng dẫn đúng MỘT hạng mục (Track).
    /// KHÔNG tạo EventRole ngay — chỉ tạo lời mời (EventRoleInvitation) chờ phản hồi và gửi email kèm
    /// link Đồng ý/Từ chối. EventRole (Mentor gắn Track) chỉ được tạo khi mentor bấm chấp nhận.
    /// </summary>
    public class InviteMentorToTrackCommandHandler : IRequestHandler<InviteMentorToTrackCommand, Result<InviteMentorToTrackResponseModel>>
    {
        ...
        public async Task<Result<InviteMentorToTrackResponseModel>> Handle(InviteMentorToTrackCommand request, CancellationToken cancellationToken)
        {
            var m = request.Model;
            ...
            if (invitedUser == null)
            {
                ...
                var verifyBody = EmailTemplate.Render(
                    ...
                    introHtml: $"Bạn được mời làm <b>cố vấn (mentor)</b> hướng dẫn hạng mục <b>{track.TrackName}</b> trong sự kiện <b>{ev.EventName}</b>. Email của bạn chưa có tài khoản — hệ thống đã tạo sẵn một tài khoản tạm.",
            ...
            var roleConflict = await EventRoleValidationHelper.CheckRoleConflictAsync(
                _unitOfWork, invitedUser.Id, m.EventId, EventRoleType.Mentor, m.TrackId, null, cancellationToken);
            if (roleConflict != null)
                return BaseException.BadRequestDupplicationResponse(roleConflict);

            // 5. Không tạo trùng lời mời còn hiệu lực (cùng vai trò Mentor cho hạng mục này)
            ...
            var hasPendingInvite = await _unitOfWork.GetRepository<EventRoleInvitation>().AnyAsync(
                i => i.InvitedUserId == invitedUser.Id && i.EventId == m.EventId
                  && i.TrackId == m.TrackId && i.RoleName == EventRoleType.Mentor
                  && i.Status == EventRoleInvitationStatus.Pending
                  && i.ExpiresAt > now,
                cancellationToken);
            if (hasPendingInvite)
                return BaseException.BadRequestInvaildInputResponse(
                    "Mentor đang có một lời mời hướng dẫn hạng mục này chờ phản hồi.");

            // 5b. Chặn NGAY TỪ LÚC MỜI: người này đang có lời mời vai trò XUNG KHẮC (EC/Giám khảo) chờ phản hồi
            ...
            var hasConflictPendingInvite = await _unitOfWork.GetRepository<EventRoleInvitation>().AnyAsync(
                i => i.InvitedUserId == invitedUser.Id && i.EventId == m.EventId
                  && (i.RoleName == EventRoleType.EventCoordinator || (i.RoleName == EventRoleType.Judge && i.TrackId == m.TrackId))
                  && i.Status == EventRoleInvitationStatus.Pending
                  && i.ExpiresAt > now,
                cancellationToken);
            if (hasConflictPendingInvite)
                return BaseException.BadRequestInvaildInputResponse(
                    "Người này đang có lời mời làm Event Coordinator hoặc làm Giám khảo hạng mục này chờ phản hồi — không thể mời làm Cố vấn.");
            ...
            var body = EmailTemplate.Render(
                ...
                greetingName: invitedUser.FullName,
                introHtml: $"Bạn được mời làm <b>mentor</b> hướng dẫn hạng mục <b>{track.TrackName}</b> trong sự kiện <b>{ev.EventName}</b>.",
                calloutLabel: "Thông tin lời mời",
                calloutHtml: $"Sự kiện: <b>{ev.EventName}</b><br>Hạng mục: <b>{track.TrackName}</b><br>Vai trò: <b>Mentor</b>",
            ...
            return new InviteMentorToTrackResponseModel
            {
                ...
                InvitedUserId = invitedUser.Id,
                MentorEmail = invitedUser.Email,
                MentorFullName = invitedUser.FullName,
                EventId = m.EventId,
                TrackId = m.TrackId,
